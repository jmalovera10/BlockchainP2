...
it('can add the star name and star symbol properly', async () => {
	...
	assert.equal('Stark', name);
	assert.equal('STK', symbol);
});
...
it('lets a user transfer a star', async () => {
	...
	const userTo = accounts[1];
	instance.transferStar(userTo, tokenId, { from: user });

	const owner = await instance.ownerOf.call(tokenId);
	assert.equal(owner, userTo);
});
...
it('lookUptokenIdToStarInfo test', async () => {
	...
	await instance.createStar('StarM', tokenId, { from: user });
	const starName = await instance.lookUptokenIdToStarInfo(tokenId);
	assert.equal('StarM', starName);
});
